...
# evaluate_naive_method(val_steps, val_gen) -> gives an average mae of 0.29

# create a cheap model to see if it's worth the complexity of an RNN
'''network = models.Sequential()
network.add(layers.Flatten(input_shape=(lookback // step, float_data.shape[-1])))
network.add(layers.Dense(32, activation='relu'))
network.add(layers.Dense(1))'''

# create a sophisticated model now
network = models.Sequential()
network.add(layers.GRU(
    32, 
    # dropout=0.2, ## change to 0.1
    # recurrent_dropout=0.2, ## change to 0.5
